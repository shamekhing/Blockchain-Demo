...
class Database:
    ...
    def add_transaction(self, transaction):
        #transaction_id = 
        self.transactions.insert_one(transaction).inserted_id
    ...
    def create_block(self, block):
        ...
        self.chain.insert_one(data)
        #_id = result.inserted_id # create block
...
#database.add_genesis(blockchain.get_prev_block())

""" 
TODO #1 genesis error # problem: add genesis block to DB
info: all have thier own genesis
initial fix: longest survive
future fix: genesis = blockchain.get_prev_block()database.create_block(genesis)
status: not solved
"""
#database.create_block(blockchain.get_prev_block())
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, receiver = 'Shamekh', amount = 1) # my reward
    prev_hash = blockchain.hash(prev_block) # genesis has no hash till this line
    block = blockchain.create_block(proof, prev_hash) 
    database.create_block(block)
...
def get_chain():
    #chain = json.dumps(json.JSONDecoder().decode(database.chain.find()))
